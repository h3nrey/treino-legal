...
from db.repositories.exercise_repo import create_exercise, get_exercise, get_exercises, update_exercise, delete_exercise, search_exercises
router = APIRouter()

@router.get("/exercise/{exercise_id}", response_model=ExercisePage)
...
async def search_existing_exercises(skip: int = 0, limit: int = 100, sort_by: str = "name", asc: bool = True, filters: list[str] = Query(None), search_query: str = None, db: Session = Depends(get_db)):
    exercises = await search_exercises(db, skip, limit, sort_by, asc, filters, search_query)
    return exercises
...
@router.delete("/exercise/{exercise_id}", response_model=str)
