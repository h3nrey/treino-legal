  ...
  loadExercises(exerciseId: string) {
    this.exerciseService.getExerciseById(exerciseId).subscribe((data) => {
      ...
      this.exerciseData = {
        ...
        primaryMuscle: data.usedMuscles.filter((muscle: exerciseMuscle) => muscle.isPrimary)[0]
          .name,
        otherMuscles: data.usedMuscles
          .filter((muscle: exerciseMuscle) => !muscle.isPrimary)
          .map((muscle: exerciseMuscle) => muscle.name),
      };
    });
  }
}
